...
class SintegraRegister(object):
    ...
    def _arg_to_string(self, value, length, argtype):
        ...
        if argtype == _number_type:
            ...
            if value > max_value:
                value = max_value
            arg = b'%0*d' % (length, value)
        elif argtype == bytes:
            arg = b'%-*s' % (length, value)
