...
# InheritableSQLObject instances need to know what is the connection of the
...
# instantiate InheritableSQLObject there is a recursive call to its
...
class AbstractModel:
    ...
    def is_parent_kwargs(self, kwargs):
        """When adding code in __init___ method of a InheritableSQLObject
    ...
    def setup_inheritable_constructor(self, kwargs):
        if not isinstance(self, InheritableSQLObject):
            raise TypeError('This method can only be called by '
                            'InheritableSQLObject instances, got %s'
                            % type(self))
        # FIXME waiting for bug fix in SQLObject
...
class Domain(BaseDomain, Componentized):
    ...
    def addFacet(self, iface, *args, **kwargs):
        ...
        if func:
            ...
            if issubclass(adapterClass, InheritableSQLObject):
                # FIXME waiting for bug fix in SQLObject
                assert kwargs.has_key('connection')
                assert kwargs['connection'] is not None
...
class InheritableModel(InheritableSQLObject,  AbstractModel):
    ...
    def __init__(self, *args, **kwargs):
        self.setup_inheritable_constructor(kwargs)
...
# FIXME Waiting for SQLObject bug fix. We can't use the same conection 
# used in callsites when creating new objects inherited from 
