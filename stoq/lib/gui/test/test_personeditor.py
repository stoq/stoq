...
from stoq.lib.gui.editors.personeditor import (ClientEditor, UserEditor,
                                               EmployeeRoleEditor,
...
from stoq.lib.gui.test.uitestutils import GUITest, GUIDumper
...
class TestClientEditor(_BasePersonEditorTest):
    ...
    @mock.patch('stoq.lib.gui.templates.persontemplate.warning')
    ...
    def test_edit_address(self, warning):
        ...
        def run_dialog(dialog, parent, *args, **kwargs):
            d = dialog(*args, **kwargs)
            dump.dump_dialog(d)
            with mock.patch('stoq.lib.gui.base.lists.run_dialog',
                            new=run_dialog2):
                self.click(d.list_slave.listcontainer.add_button)
            return d.retval

        with mock.patch('stoq.lib.gui.templates.persontemplate.run_dialog',
                        new=run_dialog):
            self.click(editor.get_person_slave().address_button)
