...
from stoq.lib.gui.wizards.loanwizard import CloseLoanWizard, NewLoanWizard
from stoqlib.lib.dateutils import localdatetime, localtoday
from stoqlib.lib.defaults import MAX_INT


class TestNewLoanWizard(GUITest):
    @mock.patch('stoq.lib.gui.wizards.loanwizard.print_report')
    @mock.patch('stoq.lib.gui.wizards.loanwizard.yesno')
...
class TestCloseLoanWizard(GUITest):
    @mock.patch('stoq.lib.gui.wizards.loanwizard.info')
    def test_confirm(self, info):
        ...
        module = 'stoq.lib.gui.events.CloseLoanWizardFinishEvent.emit'
        with mock.patch(module) as emit:
            self.click(wizard.next_button)
            self.assertEqual(emit.call_count, 1)
            args, kwargs = emit.call_args
